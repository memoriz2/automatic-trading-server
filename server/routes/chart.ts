...
export function registerChartRoutes(app: Express): void {

    // 김치 프리미엄 차트 데이터 조회 (오전 9시부터) - 인증 불필요
    app.get("/api/chart/kimchi-premium", async (req: any, res) => {
      try {
    ...
    // 김치 프리미엄 데이터 저장
    app.post("/api/chart/kimchi-premium", authenticateSession, async (req: any, res) => {
      try {
