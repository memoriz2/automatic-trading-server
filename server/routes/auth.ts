...
//       const token = generateToken(user.id, user.username);
  ...
  app.get("/api/admin/users", authenticateAdmin, async (_req: any, res) => {
    try {
      ...
      const safeUsers = await Promise.all(users.map(async (user) => {
        ...
        let apiChangeStatus: 'pending' | 'approved' = 'approved';
        try {
          const exchanges = await storage.getExchangesByUserId(user.id);
          // 하나라도 pending이 있으면 pending으로 표시
          const hasPendingChange = exchanges.some((ex: any) => ex.apiChangeStatus === 'pending');
          if (hasPendingChange) {
  ...
  // API 변경 승인 (관리자 전용)
  app.post("/api/admin/users/:userId/approve-api", authenticateAdmin, async (req: any, res): Promise<void> => {
    try {
      const userId = parseInt(req.params.userId);
      if (isNaN(userId)) {
        res.status(400).json({ error: "유효하지 않은 사용자 ID입니다" });
