...
export class BinanceAdapter extends BaseExchangeAdapter {
  ...
  readonly isTestnet = process.env.NODE_ENV !== 'production';
  
  private baseUrl = this.isTestnet 
    ? 'https://testnet.binance.vision'
    : 'https://api.binance.com';
  
  private futuresBaseUrl = this.isTestnet
...
//       const futuresAccount = await this.apiRequest('/fapi/v2/account', 'GET', {}, true);
    ...
    // 현물 계정 정보 조회 시도 (선택사항)
    try {
      const accountInfo = await this.apiRequest<{
      ...
      hasAnyAccess = true;
      if (accountInfo.canTrade) permissions.push('spot');
      ...
      // 선물이 성공했다면 현물 실패는 무시
      if (!hasAnyAccess) lastError = error;
    }
  ...
  async getBalances(): Promise<BalanceDto[]> {
    ...
    // 현물 잔고 조회
    try {
      const spotAccount = await this.apiRequest<{
        balances: Array<{
          asset: string;
          free: string;
          locked: string;
    ...
    try {
      const futuresAccountInfo = await this.apiRequest<{
  ...
  async getCurrentPrice(symbol: string): Promise<number> {
    ...
    const ticker = await this.apiRequest<{
      symbol: string;
      price: string;
    }>(`/api/v3/ticker/price?symbol=${normalizedSymbol}`);
  ...
  async getOrderBook(symbol: string, depth: number = 10): Promise<{
    ...
    const orderbook = await this.apiRequest<{
      ...
      bids: string[][];
      asks: string[][];
    }>(`/api/v3/depth?symbol=${normalizedSymbol}&limit=${depth}`);
  ...
  async createOrder(orderRequest: OrderRequestDto): Promise<OrderResponseDto> {
    ...
    }>('/api/v3/order', 'POST', orderData);

    const totalFee = result.fills.reduce((sum, fill) => 
    ...
    return {
      ...
      averagePrice: result.fills.length > 0 
        ? result.fills.reduce((sum, fill, _, arr) => 
            sum + (parseFloat(fill.price) * parseFloat(fill.qty)) / arr.length, 0
          )
        : undefined,
      fee: totalFee,
   ...
   * 포지션 조회 (선물 전용)
   */
  async getPositions(symbol?: string): Promise<Array<{
    symbol: string;
    side: 'long' | 'short';
    size: number;
    entryPrice: number;
    ...
    const positions = await this.apiRequest<Array<{
      symbol: string;
      positionAmt: string;
      entryPrice: string;
      markPrice: string;
  ...
  async getMinOrderSize(symbol: string): Promise<number> {
    ...
    const exchangeInfo = await this.apiRequest<{
      symbols: Array<{
        symbol: string;
        filters: Array<{
          filterType: string;
   ...
   */
  private mapBinanceStatus(binanceStatus: string): 'pending' | 'filled' | 'partially_filled' | 'cancelled' | 'rejected' {
   ...
   * 현물 계정 정보 조회
   */
  async getSpotAccountInfo(): Promise<{
    makerCommission: number;
    takerCommission: number;
    buyerCommission: number;
    sellerCommission: number;
   ...
   * 선물 계정 정보 조회
   */
  async getFuturesAccountInfo(): Promise<{
    feeTier: number;
    canTrade: boolean;
    canDeposit: boolean;
    canWithdraw: boolean;
   ...
   * 선물 주문 생성
   */
  async createFuturesOrder(orderRequest: OrderRequestDto): Promise<OrderResponseDto> {
    const normalizedSymbol = this.normalizeSymbol(orderRequest.symbol);
    ...
    const result = await this.apiRequest<{
      ...
      origType: string;
      updateTime: number;
    }>('/fapi/v1/order', 'POST', orderData, true);
