...
export class DatabaseStorage {
  ...
  async getActivePositionByStrategy(strategyId: number, symbol: string, userId?: number): Promise<any | undefined> {
    try {
      ...
      const params: any[] = [strategyId, symbol, 'open', 'pending'];
      ...
      if ((data.side === 'sell' || data.side === 'cover') && !data.positionId) {
        ...
        try {
          ...
          const matchingPosition = positions.find((p: any) => {
            if (p.symbol !== data.symbol) return false;
            // 청산 타입에 따라 매칭
            if (data.side === 'sell' && data.exchange === 'upbit') return p.upbit_quantity > 0;
            if (data.side === 'cover' && data.exchange === 'binance') return p.binance_quantity < 0;
            return false;
          });
