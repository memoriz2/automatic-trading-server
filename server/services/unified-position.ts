...
export interface PositionEntryParams {
  ...
  strategyId: number;
  symbol: string;
  side: 'buy' | 'sell' | 'short' | 'cover' | 'long';
  exchange: 'upbit' | 'binance';
  quantity: number;
  price: number;
  premiumRate: number;
  entryUsdKrw?: number; // 진입 시점 USD/KRW 환율
...
export class UnifiedPositionService {
  private positionsRepo = new PositionsRepository();
  private tradesRepo = new TradesRepository();

  /**
   * 안전한 포지션 진입 (중복 방지 포함)
   */
  async safeEnterPosition(params: PositionEntryParams): Promise<PositionEntryResult> {
    try {
      ...
      const existingPosition = await this.positionsRepo.getOpenPositionByUserStrategyAndSymbol(
        params.userId,
        params.strategyId,
        params.symbol
      );
      ...
      const baseData = {
        ...
        side: (params.side === 'buy' ? 'short' : params.side === 'sell' ? 'long' : (params.side as 'long' | 'short')),
      ...
      try {
        position = await this.positionsRepo.createOpenIfNone(baseData as any);
   ...
   * 전략별 활성 포지션 조회
   */
  async getActivePositionsByStrategy(strategyId: number): Promise<any[]> {
    try {
   ...
   * 사용자별 활성 포지션 조회
   */
  async getActivePositionsByUser(userId: number): Promise<any[]> {
    try {
      const positions = await storage.getPositions({ 
        user_id: userId, 
        status: 'open' 
   ...
   */
  async checkEntryAllowed(strategyId: number, symbol: string): Promise<{
    allowed: boolean;
    message: string;
    existingPosition?: any;
  }> {
