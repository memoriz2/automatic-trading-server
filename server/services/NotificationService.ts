...
export class NotificationService extends BaseRepository {
   ...
   * 오류 발생 시 알림 발송
   */
  async sendErrorNotification(tradingError: TradingErrorDto): Promise<void> {
    try {
      // 알림 발송 여부 확인
   ...
   * 알림 발송 여부 결정
   */
  private shouldSendNotification(tradingError: TradingErrorDto): boolean {
    // 심각도 기반 필터링
   ...
   * 알림 제한 확인 (스팸 방지)
   */
  private isThrottled(tradingError: TradingErrorDto): boolean {
    if (!tradingError.errorSignature) {
      return false;
    }
   ...
   * 슬랙 알림 발송
   */
  private async sendSlackNotification(tradingError: TradingErrorDto): Promise<void> {
   ...
   * 이메일 알림 발송
   */
  private async sendEmailNotification(tradingError: TradingErrorDto): Promise<void> {
   ...
   * 웹훅 알림 발송
   */
  private async sendWebhookNotification(tradingError: TradingErrorDto): Promise<void> {
   ...
   * 대시보드 알림 (실시간 업데이트)
   */
  private async sendDashboardNotification(tradingError: TradingErrorDto): Promise<void> {
   ...
   * 슬랙 메시지 포맷팅
   */
  private formatSlackMessage(tradingError: TradingErrorDto): any {
   ...
   * 대량 알림 발송 (배치 처리)
   */
  async sendBatchNotifications(tradingErrors: TradingErrorDto[]): Promise<{
    success: number;
    failed: number;
    skipped: number;
  }> {
  ...
  async retryFailedNotifications(): Promise<void> {
    try {
      ...
      const failedNotifications = await this.queryMany<any>(query, []);
      ...
      for (const notification of failedNotifications) {
        try {
          // 원본 오류 정보 재구성
          const tradingError: TradingErrorDto = {
   ...
   * 알림 상태 업데이트
   */
  private async updateNotificationStatus(notificationId: number, status: 'sent' | 'failed'): Promise<void> {
    const query = `
  ...
  async getNotificationStats(days: number = 7): Promise<{
    ...
    const result = await this.queryOne<any>(query, [days]);
