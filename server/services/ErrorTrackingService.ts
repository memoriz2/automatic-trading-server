...
export class ErrorTrackingService {
   ...
   * 거래 오류 기록 및 분석
   */
  async recordError(request: CreateErrorRequest): Promise<TradingErrorDto> {
    ...
    // 거래 오류 데이터 구성
    const errorData: Omit<TradingErrorDto, 'id' | 'createdAt' | 'updatedAt'> = {
   ...
   * 오류 분석
   */
  private async analyzeError(error: Error | string, context: any): Promise<ErrorAnalysisDto> {
   ...
   * 오류 심각도 결정
   */
  private determineSeverity(message: string, code?: string, category?: ErrorCategory): ErrorSeverity {
    const lowerMessage = message.toLowerCase();
   ...
   * 추천 재시도 설정
   */
  private getRecommendedRetryConfig(category: ErrorCategory, severity: ErrorSeverity): RetryConfig {
   ...
   * 재시도 실행
   */
  async retryError(errorId: number): Promise<{ success: boolean; result?: any; error?: string }> {
   ...
   * 실제 재시도 로직 실행
   */
  private async executeRetry(tradingError: TradingErrorDto): Promise<any> {
   ...
   * 오류 시그니처 생성
   */
  private generateErrorSignature(error: Error | string, context: any): string {
   ...
   * 오류 패턴 업데이트
   */
  private async updateErrorPattern(signature: string, tradingError: TradingErrorDto): Promise<void> {
    try {
   ...
   * 수정 우선순위 결정
   */
  private getFixPriority(severity: ErrorSeverity): number {
    switch (severity) {
   ...
   * 오류 알림 발송
   */
  private async sendErrorNotification(tradingError: TradingErrorDto): Promise<void> {
   ...
   * 오류 통계 조회
   */
  async getErrorStats(userId?: number, days: number = 30) {
   ...
   * 사용자별 오류 목록 조회
   */
  async getUserErrors(userId: number, options: any = {}) {
   ...
   * 오류 패턴 목록 조회
   */
  async getErrorPatterns(options: any = {}) {
