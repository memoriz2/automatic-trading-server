...
// 🔐 전역 함수 타입 정의
declare global {
  var notifySessionInvalidated: ((userId: number) => boolean) | undefined;
}
    ...
    if (position.symbol === 'BTC') {
      try {
        ...
        if (btcBalance > TRADING_CONSTANTS.BTC_MIN_QUANTITY) { // 최소 거래 단위 체크
          ...
          // 업비트 주문 상세 조회하여 실제 체결가/수수료 확인
          await new Promise(resolve => setTimeout(resolve, 1000));
          const orderDetail = await upbitService.getOrderDetail(sellResult.uuid);
          const actualPrice = parseFloat(orderDetail.avg_price || orderDetail.price || '0');
          const actualQuantity = parseFloat(orderDetail.executed_volume || '0');
          const actualFee = parseFloat(orderDetail.paid_fee || '0');
          ...
          if (actualFee > 0) {
            await storage.createTrade({
              ...
              price: actualPrice,
              fee: actualFee,
              exchangeOrderId: sellResult.uuid
            });
            logInfo('업비트 매도 거래 기록 저장 완료', {
              orderId: sellResult.uuid,
              fee: actualFee
            });
          } else {
            logInfo('⚠️ 업비트 매도 수수료 0이므로 거래 기록 저장 안 함', {
              orderId: sellResult.uuid
            });
          }
    ...
    try {
      ...
      if (accountInfo.positions && accountInfo.positions.length > 0) {
        ...
        if (btcPosition) {
          ...
          // 바이낸스 주문 상세 조회하여 실제 체결가/수수료 확인
          await new Promise(resolve => setTimeout(resolve, 1000));
          const binanceOrderDetail = await binanceService.getFuturesOrderDetail('BTC', closeResult.orderId);
          const binancePrice = parseFloat(binanceOrderDetail.avgPrice || binanceOrderDetail.price || '0');
...
export async function registerRoutes(
  ...
  // 활성 포지션 조회
  app.get("/api/positions/:userId", async (req, res): Promise<void> => {
    try {
      ...
      try {
        ...
        if (services.binanceService) {
          ...
          console.log('📊 바이낸스 포지션 맵:', Array.from(binancePositionsMap.entries()).map(([sym, positions]) =>
            `${sym}: ${positions.length}개 (${positions.map(p => `${p.positionAmt} BTC`).join(', ')})`
          ));
        }
      } catch (error) {
      ...
      const mappedPositions = positions.map(p => {
        ...
        return {
          ...
          binanceEntryPrice: Number(p.binance_entry_price) || 0, // DB 값만 사용 (API 값 사용 안함)
  ...
  // 포지션 업비트 수량 업데이트 API
  // 포지션 업데이트 (청산, 수량 변경 등)
  app.put("/api/positions/:id", authenticateSession, async (req: any, res): Promise<void> => {
    try {
  ...
  app.put("/api/positions/:id/upbit-quantity", authenticateSession, async (req: any, res): Promise<void> => {
    try {
  ...
  // 자동매매 상태 조회 (전체)
  app.get("/api/trading/status", async (req, res): Promise<void> => {
    try {
  ...
  // 거래소 API 키 설정
  app.post("/api/exchanges/:userId", async (req, res): Promise<void> => {
  ...
  // 잔고 조회
  app.get("/api/balances/:userId", authenticateSession, async (req: any, res): Promise<void> => {
    try {
      const userId = req.user.id; // 인증된 사용자 ID 사용
      // 잔고 조회 시작
  ...
  // 거래 전략 수정 (PUT)
  app.put("/api/trading-strategies/:id", authenticateSession, async (req: any, res): Promise<void> => {
    try {
      const authenticatedUserId = req.user.id; // 인증된 사용자 ID 사용
  ...
  const userWebSocketMap = new Map<number, WebSocket>();
  ...
  global.notifySessionInvalidated = (userId: number) => {
  ...
  app.post("/api/test-exchange-connection", authenticateSession, async (req: any, res): Promise<void> => {
  ...
  app.get("/api/me/positions", authenticateSession, async (req: any, res) => {
    try {
      ...
      let binancePositionsMap: Map<string, any> = new Map();
      ...
      if (status === 'open' && positions.length > 0) {
        try {
          ...
          if (exchange?.apiKey && exchange?.apiSecret) {
            ...
            binancePositions.forEach((pos: any) => {
      ...
      const mappedPositions = positions.map((p: any) => {
        ...
        return {
          ...
          // 포지션 수량/가격
          upbitPrice: num(p.upbitPrice ?? p.upbitprice ?? p.entry_price), // 현재가 (deprecated, 호환용)
          binancePrice: num(p.binancePrice ?? p.binanceprice ?? p.binance_entry_price), // 현재가 (deprecated, 호환용)
          upbitEntryPrice: num(p.upbitEntryPrice ?? p.entry_price), // 업비트 진입 총액
          binanceEntryPrice: num(p.binance_entry_price ?? p.binancePrice ?? p.binanceEntryPrice), // 바이낸스 진입 총액 (DB 컬럼 직접 우선)
          upbitQuantity: num(p.upbitQuantity ?? p.upbitquantity ?? p.quantity),
          binanceQuantity: num(p.binanceQuantity ?? p.binancequantity ?? p.binance_quantity),
  ...
  // 어드민 권한 확인 API
  app.get("/api/admin/check", authenticateSession, async (req: any, res): Promise<void> => {
    try {
      const userId = req.user.id;
  ...
  // 거래소 API 연결 상태 확인
  app.get("/api/exchanges/status", authenticateSession, async (req: any, res): Promise<void> => {
    try {
      const userId = req.user.id;
  ...
  // 거래소 연결 테스트
  app.post("/api/v2/exchanges/test", authenticateSession, async (req: any, res): Promise<void> => {
    try {
      const userId = req.user.id;
  ...
  // API 키 저장
  app.post("/api/v2/exchanges/connect", authenticateSession, async (req: any, res): Promise<void> => {
    try {
      const userId = req.user.id;
  ...
  // 업비트 잔고 직접 조회 테스트
  app.get("/api/test/upbit-balance", authenticateSession, async (req: any, res): Promise<void> => {
    try {
      const userId = req.user.id;
  ...
  app.get("/api/test/binance-balance", authenticateSession, async (req: any, res): Promise<void> => {
    try {
      const userId = req.user.id;
  ...
  // 업비트 BTC 매수 주문
  app.post("/api/trading/upbit/buy", authenticateSession, async (req: any, res): Promise<void> => {
    try {
      const userId = req.user.id;
      ...
      try {
        ...
        if (strategyId) {
          ...
          if (activePosition) {
            ...
            console.log(`⚠️ [Upbit Buy] 포지션 없음 - positionId=null로 저장`);
          }
        }
        ...
        if (orderDetail.trades && orderDetail.trades.length > 0) {
          ...
          orderDetail.trades.forEach((trade: any) => {
        ...
        if (paidFee > 0) {
          await storage.createTrade({
            ...
            price: perBtcPrice, // ✅ FIX: avgPrice 대신 perBtcPrice 사용
            fee: paidFee,
            feeCurrency: 'KRW',
            exchangeOrderId: orderResult.uuid,
            executedAt: new Date(),
            isMock: false
          });
          console.log(`✅ 업비트 거래 기록 DB 저장 성공 (price: ${perBtcPrice.toLocaleString()}원, fee: ${paidFee})`);
        } else {
          console.log(`⚠️ 업비트 매수 수수료 0이므로 거래 기록 저장 안 함 (uuid: ${orderResult.uuid})`);
        }
  ...
  app.post("/api/trading/binance/close-short", authenticateSession, async (req: any, res): Promise<void> => {
    try {
      const userId = req.user.id;
      ...
      try {
        // 바이낸스 주문 상세 조회
        await new Promise(resolve => setTimeout(resolve, 1000));
        const orderDetail = await binanceService.getFuturesOrderDetail(symbol.replace('USDT', ''), closeResult.orderId);

        const avgPrice = parseFloat(orderDetail.avgPrice || orderDetail.price || "0");
        const executedQty = parseFloat(orderDetail.executedQty || "0");
        const commission = parseFloat(orderDetail.commission || "0");

        console.log(`📊 바이낸스 청산 상세 정보:`, {
          orderId: closeResult.orderId,
          avgPrice,
          executedQty,
          commission
...
(async () => {
  ...
  app.get("/api/trading/binance/account-info", authenticateSession, async (req: any, res): Promise<void> => {
    try {
      const userId = req.user.id;
...
(async () => {
  ...
  // 업비트 BTC 매도 주문
  app.post("/api/trading/upbit/sell", authenticateSession, async (req: any, res): Promise<void> => {
    try {
      const userId = req.user.id;
  ...
  app.post("/api/trading/binance/short", authenticateSession, async (req: any, res): Promise<void> => {
    try {
      const userId = req.user.id;
      ...
      if (strategyId) {
        ...
        const activePositions = await storage.getActivePositions(userId);
        activePosition = activePositions.find((p: any) =>
          p.symbol === symbolOnly && p.status === 'open'
        );
      }

      if (activePosition) {
        console.log(`✅ [Binance Short] 기존 포지션 찾음: ID=${activePosition.id}`);
      } else {
        console.log(`⚠️ [Binance Short] 포지션 없음 - positionId=null로 저장`);
      }

      // 성공한 거래 기록 저장 (포지션 생성 후)
      try {
        // 바이낸스 주문 상세 조회하여 실제 체결가/수수료 가져오기
        await new Promise(resolve => setTimeout(resolve, 1000));
        const orderDetail = await binanceService.getFuturesOrderDetail(symbol.replace('USDT', ''), orderResult.orderId);

        const avgPrice = parseFloat(orderDetail.avgPrice || orderDetail.price || "0");
        const executedQty = parseFloat(orderDetail.executedQty || "0");
        const commission = parseFloat(orderDetail.commission || "0");

        console.log(`📊 바이낸스 주문 상세 정보:`, {
        ...
        if (commission > 0) {
          ...
          console.log(`✅ 바이낸스 거래 기록 DB 저장 성공 (positionId: ${activePosition?.id}, fee: ${commission})`);
        } else {
  ...
  app.get("/api/test/upbit-order/:uuid", authenticateSession, async (req: any, res): Promise<void> => {
    try {
      ...
      const orderDetail = await services.upbitService.getOrderDetail(uuid);
  ...
  app.get("/api/test/binance-order/:orderId", authenticateSession, async (req: any, res): Promise<void> => {
    try {
      ...
      // 바이낸스 선물 주문 조회 (getFuturesOrderDetail 사용)
      const orderDetail = await services.binanceService.getFuturesOrderDetail('BTC', parseInt(orderId));
      res.json(orderDetail);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.post("/api/emergency/liquidate-upbit", authenticateSession, async (req: any, res): Promise<void> => {
    try {
      const userId = req.user.id;
      ...
      // 업비트 거래소 정보 조회
      const exchanges = await storage.getExchangesByUserId(userId);
      const upbitExchange = exchanges.find((e: any) => e.exchange === 'upbit');
      ...
      const apiKey = (upbitExchange as any).api_key || (upbitExchange as any).apiKey;
      const apiSecret = (upbitExchange as any).api_secret || (upbitExchange as any).apiSecret;
      ...
      const btcAccount = accountData.find((acc: any) => acc.currency === 'BTC');
      const btcBalance = btcAccount ? parseFloat(btcAccount.balance) : 0;
  ...
  app.post("/api/emergency/liquidate-binance", authenticateSession, async (req: any, res): Promise<void> => {
    try {
      const userId = req.user.id;
      const services = await ExchangeServiceFactory.initializeByUserId(userId);
      ...
      const btcPosition = positions.find((p: any) => p.symbol === 'BTCUSDT');
  ...
  app.post("/api/emergency/liquidate-all", authenticateSession, async (req: any, res): Promise<void> => {
    try {
      const userId = req.user.id;
      const services = await ExchangeServiceFactory.initializeByUserId(userId);
      const results: any = {
