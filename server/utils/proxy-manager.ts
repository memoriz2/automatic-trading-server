...
export class ProxyManager {
  ...
  private readonly MAX_ERRORS_PER_PROXY = 3;
  private readonly PROXY_COOLDOWN_MS = 5 * 60 * 1000; // 5분
  ...
  private loadProxyConfig(): void {
    ...
    if (proxyList) {
      try {
        ...
        this.proxies = proxies.map((proxy: any) => ({
          ...proxy,
          isActive: true,
          lastUsed: 0,
          errorCount: 0,
   ...
   * 프록시 에러 기록
   */
  recordProxyError(proxy: ProxyConfig, _error: Error): void {
    proxy.errorCount++;
   ...
   * 프록시 성공 기록 (에러 카운트 리셋)
   */
  recordProxySuccess(proxy: ProxyConfig): void {
   ...
   */
  createAgent(proxy: ProxyConfig | null): any {
    if (!proxy) {
