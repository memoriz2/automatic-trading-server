 ...
 * 표준화된 거래 에러 클래스
 */
export class StandardTradingError extends Error implements TradingError {
  public readonly code: string;
  public readonly exchange?: 'upbit' | 'binance';
...
export class ErrorHandler {
  ...
  static insufficientBalance(
    ...
    required: number,
    available: number
  ): StandardTradingError {
    return new StandardTradingError(
      ERROR_CODES.INSUFFICIENT_BALANCE,
      `${exchange} ${currency} 잔고 부족: 필요=${required}, 보유=${available}`,
      {
        exchange,
  ...
  static orderFailed(
    ...
    return new StandardTradingError(
      ERROR_CODES.ORDER_FAILED,
      `${exchange} 주문 실패: ${message}`,
      {
        exchange,
        orderId,
  ...
  static rateLimitExceeded(
    ...
    return new StandardTradingError(
      ...
      `${exchange} API 호출 제한 초과${endpoint ? `: ${endpoint}` : ''}`,
      {
        exchange,
        details: { endpoint }
      }
  ...
  static authenticationFailed(
    ...
    return new StandardTradingError(
      ...
      `${exchange} 인증 실패${message ? `: ${message}` : ''}`,
      {
        exchange,
        details: { originalMessage: message }
      }
  ...
  static invalidOrderSize(
    ...
    return new StandardTradingError(
      ERROR_CODES.INVALID_ORDER_SIZE,
      `${exchange} ${symbol} 최소 거래 ${unit} 미달: ${provided} < ${minimum}`,
      {
        exchange,
    ...
    if (error instanceof Error) {
      ...
      if (message.includes('insufficient') && message.includes('balance')) {
        return new StandardTradingError(
          ERROR_CODES.INSUFFICIENT_BALANCE,
          error.message,
          { exchange: context?.exchange, cause: error }
        );
      }
   ...
   * 에러 로깅 (구조화된 로그)
   */
  static logError(error: StandardTradingError, userId?: number): void {
    const logData = {
   ...
   * 재시도 가능한 에러인지 판단
   */
  static isRetryable(error: StandardTradingError): boolean {
   ...
   */
  static getUserFriendlyMessage(error: StandardTradingError): string {
    const exchangeNames = {
      upbit: '업비트',
      binance: '바이낸스'
    };
