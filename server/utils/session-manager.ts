...
export async function invalidateUserSessions(
  ...
  try {
    ...
    let deviceInfo = '';
    if (user) {
      const deviceTypeEmoji: Record<string, string> = {
        mobile: '📱',
        desktop: '🖥️',
        tablet: '💻',
        unknown: '❓'
      };
      const row: any = await (storage as any).pool.query(
        'SELECT last_login_device_type, last_login_ip, last_login_browser, last_login_os FROM users WHERE id = $1',
        [userId]
      ).then((r: any) => r.rows[0]);
