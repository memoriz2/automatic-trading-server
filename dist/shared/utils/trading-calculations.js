 ...
 */
export function normalizeAmountBtc(raw, upbitPrice) {
    ...
    if (!isFinite(amt) || amt <= 0)
        amt = 0.001;
    if (amt > 10)
        amt = 0.001; // 상식적 한도 초과 시 최소값
    return Math.max(0.001, amt);
}
/**
 * 서버 전략 → UI 밴드 매핑
 */
export function mapStrategyToBand(s) {
    return {
        name: s?.name,
 ...
 * @returns 진입 거래 계산 결과
 */
export function calculateEntryTrade(baseAmount, leverage, upbitPrice, binancePrice, entryUsdKrw) {
    // 1단계: 바이낸스 선물 숏 포지션
 ...
 * @returns 청산 거래 계산 결과
 */
export function calculateExitTrade(position, currentUpbitPrice, currentBinancePrice, currentUsdKrw, ratio = 1.0) {
    // 청산할 수량 계산
 ...
 * @returns PnL 계산 결과
 */
export function calculatePositionPnL(position, marketData) {
    ...
    const currentBinancePriceUsd = (currentBinancePriceRaw || 0) > 1000000
        ? currentBinancePriceRaw / usdkrw
        : currentBinancePriceRaw;
 ...
 * 수익률 계산
 */
export function calculateProfitRate(mockBalance, currentKimchiData) {
 ...
 * 일일 통계 계산
 */
export function calculateDailyStats(mockTrades, mockPositions) {
 ...
 * 활성 포지션 수량 계산
 */
export function calculateOpenQuantities(mockPositions) {
 ...
 * 최근 거래 필터링
 */
export function getRecentTrades(mockTrades, strategies) {
    return mockTrades
