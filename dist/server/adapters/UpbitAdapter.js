...
export class UpbitAdapter extends BaseExchangeAdapter {
    ...
    async getCurrentPrice(symbol) {
        ...
        const tickers = await this.apiRequest(`/v1/ticker?markets=${normalizedSymbol}`);
        if (tickers.length === 0) {
    ...
    async getOrderBook(symbol, depth = 10) {
        const normalizedSymbol = this.normalizeSymbol(symbol);
        const orderbook = await this.apiRequest(`/v1/orderbook?markets=${normalizedSymbol}`);
    ...
    async createOrder(orderRequest) {
        ...
        const result = await this.apiRequest('/v1/orders', 'POST', orderData);
        return {
            id: 0, // DB에서 생성될 ID
    ...
    async getOrder(orderId) {
        const result = await this.apiRequest('/v1/order', 'GET', { uuid: orderId });
        return {
            id: 0, // DB ID는 별도 관리
            userId: 0, // 별도 설정 필요
            exchange: 'upbit',
    ...
    async getActiveOrders(symbol) {
        ...
        const orders = await this.apiRequest('/v1/orders', 'GET', params);
    ...
    async getTrades(symbol, limit = 100) {
        ...
        const sortedByTime = [...orders].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
     ...
     * 캔들 데이터 조회
     */
    async getCandles(symbol, interval = 'minutes', count = 200) {
        const normalizedSymbol = this.normalizeSymbol(symbol);
