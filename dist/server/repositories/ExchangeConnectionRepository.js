...
export class ExchangeConnectionRepository extends BaseRepository {
     ...
     * 연결 상태 업데이트 또는 생성
     */
    async upsertConnection(userId, connectionData) {
        const query = `
    ...
    `;
        return this.queryOne(query, [userId, exchange]);
    }
    /**
     * 연결 상태 요약 조회 (API 응답용)
     */
     ...
     * 연결 상태만 업데이트
     */
    async updateConnectionStatus(userId, exchange, connected, error) {
        const updates = {
            connected,
        ...
        const updatedRows = await this.safeUpdate('exchange_connections', updates, { user_id: userId, exchange });
        return updatedRows > 0;
    }
    /**
     * 권한 정보 업데이트
     */
    async updatePermissions(userId, exchange, permissions, balanceAvailable, tradingEnabled) {
        const updates = {
     ...
     * 거래 가능한 거래소 목록 조회
     */
    async getTradingEnabledExchanges(userId) {
        const query = `
      SELECT exchange
      FROM exchange_connections 
     ...
     * 연결 오류 기록
     */
    async recordError(userId, exchange, error) {
        await this.safeUpdate('exchange_connections', {
            connected: false,
            error,
            last_checked: new Date()
     ...
     */
    async getSystemConnectionStatus() {
        const query = `
      SELECT 
