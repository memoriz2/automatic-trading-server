...
import { ErrorSeverity, ErrorCategory } from '../types/error-tracking.js';
/**
 * 거래 오류 추적 Repository
 */
export class ErrorTrackingRepository extends BaseRepository {
     ...
     * 새 거래 오류 기록 생성
     */
    async createTradingError(errorData) {
        const query = `
    ...
    `;
        const values = [
            ...
            errorData.errorSignature || null, // 타입에 추가 필요
            errorData.exchange,
            errorData.symbol,
            errorData.side,
     ...
     * 사용자별 거래 오류 목록 조회
     */
    async findTradingErrorsByUserId(userId, options = {}) {
        let baseQuery = `
      SELECT 
    ...
    `;
        ...
        const [errors, countResult] = await Promise.all([
            this.queryMany(baseQuery, params.slice(0, paramIndex - (options.offset ? 1 : 0))),
            this.queryOne(countQuery, params.slice(0, paramIndex - (options.limit ? 1 : 0) - (options.offset ? 1 : 0)))
        ]);
        return {
            errors,
     ...
     * 거래 오류 업데이트
     */
    async updateTradingError(id, updates) {
        const setClause = [];
        const params = [];
        let paramIndex = 1;
        // 업데이트할 필드들 동적으로 구성
     ...
     * 오류 패턴 생성 또는 업데이트
     */
    async upsertErrorPattern(patternData) {
        const query = `
     ...
     * 재시도 이력 생성
     */
    async createRetryHistory(retryData) {
        const query = `
