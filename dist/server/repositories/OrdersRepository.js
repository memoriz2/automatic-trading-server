...
export class OrdersRepository extends BaseRepository {
     ...
     * 거래소 주문 ID로 조회
     */
    async findByExchangeOrderId(exchange, exchangeOrderId) {
        const query = `
      SELECT 
        id,
        user_id as "userId",
    ...
    `;
        return this.queryOne(query, [exchange, exchangeOrderId]);
    }
    /**
     ...
     * 주문 상태 업데이트
     */
    async updateStatus(id, status, filledQuantity, averagePrice, fee) {
     ...
     * 미체결 주문 조회
     */
    async findPendingOrders(userId, exchange) {
        let query = `
      SELECT 
        id,
        user_id as "userId",
     ...
     */
    async cancel(id) {
        const updatedRows = await this.safeUpdate('orders', { status: 'cancelled' }, { id });
        return updatedRows > 0;
    }
    /**
     * 거래소별 일일 주문 통계
