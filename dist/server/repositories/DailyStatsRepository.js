...
export class DailyStatsRepository extends BaseRepository {
     ...
     * 일일 통계 조회 또는 생성
     */
    async getOrCreateDailyStats(userId, date) {
        // 먼저 기존 통계 조회
     ...
     * 거래 발생시 통계 업데이트
     */
    async incrementTradeCount(userId, date, exchange, volume, fee) {
     ...
     * 포지션 수 업데이트
     */
    async updateActivePositions(userId, date) {
        // 실제 활성 포지션 수 계산
        const activeCount = await this.queryOne('SELECT COUNT(*) as count FROM positions WHERE user_id = $1 AND status = $2 AND is_mock = false', [userId, 'open']);
     ...
     * 실현 손익 업데이트
     */
    async updateRealizedPnl(userId, date, pnlChange) {
        await this.query(`
     ...
     * 미실현 손익 업데이트
     */
    async updateUnrealizedPnl(userId, date, totalUnrealizedPnl) {
        await this.query(`
     ...
     * 승률 계산 및 업데이트
     */
    async updateWinRate(userId, date) {
        const winRateQuery = `
    ...
    `;
        const result = await this.queryOne(winRateQuery, [userId, date]);
     ...
     * 최대 손실폭 업데이트
     */
    async updateMaxDrawdown(userId, date, drawdown) {
        await this.query(`
    ...
    `;
        const result = await this.queryOne(query, [userId, year, month]);
        return {
