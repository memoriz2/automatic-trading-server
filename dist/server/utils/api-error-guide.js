 ...
 */
// 업비트 API 오류 가이드
export const UPBIT_ERROR_GUIDES = {
...
// 바이낸스 API 오류 가이드
export const BINANCE_ERROR_GUIDES = {
    'INVALID_API_KEY': {
...
// 일반적인 네트워크/연결 오류 가이드
export const GENERAL_ERROR_GUIDES = {
    'NETWORK_ERROR': {
 ...
 */
export function getApiErrorGuide(exchange, error) {
    ...
    if (statusCode === 401 || statusCode === 403) {
        if (exchange === 'upbit') {
            if (errorMessage.includes('IP'))
                return guides.IP_NOT_ALLOWED;
            if (errorMessage.includes('permission') || errorMessage.includes('권한'))
                return guides.PERMISSION_DENIED;
            return guides.INVALID_PARAMETER_APIKEY;
        }
        else {
            if (errorMessage.includes('IP'))
                return guides.IP_RESTRICTION;
            if (errorMessage.includes('signature'))
                return guides.INVALID_SIGNATURE;
            if (errorMessage.includes('disabled'))
                return guides.API_KEY_DISABLED;
