...
export class IpBanDetector {
     ...
     * 에러 메시지에서 IP 밴 감지
     */
    detectBan(exchange, error) {
        const errorMessage = typeof error === 'string' ? error : error.message;
        const isBanError = this.BAN_INDICATORS.some(indicator => errorMessage.toLowerCase().includes(indicator.toLowerCase()));
        if (!isBanError) {
     ...
     * IP 밴 대응 실행
     */
    async handleBan(exchange, detection) {
