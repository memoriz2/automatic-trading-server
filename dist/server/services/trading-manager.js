...
export class TradingManager {
    ...
    // Mock 강제진입
    async executeMockForceEntry(userId, params) {
        ...
        try {
            ...
            return {
                success: false,
                message: `Mock 강제진입 실패: ${error.message}`
            };
        }
    }
    // 실거래 강제진입
    async executeRealForceEntry(userId, params) {
        ...
        try {
            ...
            // 1. 업비트 현물 매수
            const upbitOrder = await services.upbit.placeBuyOrder(`KRW-${params.symbol}`, params.quantity);
            // 2. 바이낸스 선물 숏
