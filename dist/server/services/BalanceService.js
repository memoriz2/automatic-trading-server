...
export class BalanceService {
    ...
    static CACHE_TTL_MS = 30 * 1000; // 30초 캐시
    /**
     ...
     * 거래소 연결 상태 조회
     */
    async getExchangeStatus(userId) {
        return this.exchangeConnectionRepository.getConnectionStatus(userId);
    }
    /**
     * 특정 거래소 연결 테스트
     */
    async testExchangeConnection(userId, exchange) {
        try {
            // API 키 조회
            ...
            await this.exchangeConnectionRepository.upsertConnection(userId, {
                ...
                tradingEnabled: result.success && (result.permissions.includes('spot') ||
     ...
     * API 키 저장 및 연결 테스트
     */
    async saveApiKey(userId, exchange, apiKey, secretKey, passphrase) {
        try {
            // 1. 연결 테스트 먼저 수행
            const adapter = getExchangeAdapter(exchange);
     ...
     * 잔고 스냅샷 저장 (비동기)
     */
    async saveBalanceSnapshot(userId, balances) {
        try {
     ...
     * 잔고 응답 형태로 포맷팅
     */
    formatBalanceResponse(balances, connectionResults) {
        const real = {};
        const connected = {
     ...
     */
    async refreshBalanceAfterTrade(userId, tradeDetails) {
