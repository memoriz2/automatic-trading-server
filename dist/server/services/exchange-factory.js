...
export class ExchangeServiceFactory {
     ...
     */
    static async initializeByUserId(userId) {
        try {
     ...
     */
    static initializeByCredentials(credentials) {
        const services = {};
        // 업비트 서비스 초기화
     ...
     * 특정 거래소만 초기화
     */
    static async initializeUpbitOnly(userId) {
        try {
            const exchanges = await storage.getExchangesByUserId(userId);
            const upbitExchange = exchanges.find((ex) => ex.exchange === 'upbit' && ex.isActive);
            if (upbitExchange?.apiKey && upbitExchange?.apiSecret) {
     ...
     * 특정 거래소만 초기화
     */
    static async initializeBinanceOnly(userId) {
        try {
            const exchanges = await storage.getExchangesByUserId(userId);
            const binanceExchange = exchanges.find((ex) => ex.exchange === 'binance' && ex.isActive);
            if (binanceExchange?.apiKey && binanceExchange?.apiSecret) {
     ...
     * 서비스들이 모두 초기화되었는지 검증
     */
    static validateServices(services, requireBoth = true) {
        if (requireBoth) {
