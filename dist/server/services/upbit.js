...
export class UpbitService {
    ...
    async placeSellOrder(market, volume, orderType = 'market', retries = 3) {
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                ...
                if (volume < TRADING_CONSTANTS.BTC_MIN_QUANTITY) {
                    throw ErrorHandler.invalidOrderSize('upbit', market, volume, TRADING_CONSTANTS.BTC_MIN_QUANTITY, 'BTC');
                }
                const params = {
                    market,
                    side: 'ask',
                ...
                const shouldRetry = attempt < retries && (error?.message?.includes('429') ||
