...
export async function analyzePositions(positions) {
...
export async function saveInsightToDB(userId, insightType, analysisData, insightText, recommendations) {
    const result = await pool.query(`INSERT INTO ai_insights (user_id, insight_type, analysis_data, insight_text, recommendations)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING id`, [userId, insightType, JSON.stringify(analysisData), insightText, recommendations ? JSON.stringify(recommendations) : null]);
...
if (process.argv[1] === __filename) {
    (async () => {
        ...
        try {
            ...
            const insightId = await saveInsightToDB(userId, 'profitable_analysis', positions, analysis);
