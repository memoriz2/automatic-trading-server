...
export class BinanceService {
    ...
    proxyUrl = 'https://api1.binance.com'; // 또는 다른 지역별 엔드포인트
    apiKey;
    secretKey;
     ...
     */
    async getFuturesQuantityRules(symbolWithUsdt) {
        try {
            ...
            const marketLot = info.filters?.find((f) => f.filterType === 'MARKET_LOT_SIZE');
    ...
    ensureMinNotional(qty, price, minNotional) {
        if (!minNotional)
            return true;
    ...
    async getUSDTKRWRate() {
        // 업비트에서 실시간 USDT/KRW 가격 조회 (기본값 없음)
    ...
    async placeShortOrder(symbol, quantity, targetLeverage) {
        try {
            if (!this.apiKey) {
                throw new Error('Binance API key not configured');
            }
            ...
            // 현재 포지션 정보 확인
            const positions = accountInfo.positions?.filter((p) => parseFloat(p.positionAmt) !== 0) || [];
            console.log('📈 현재 활성 포지션:', positions.length > 0 ? positions.map((p) => ({
                symbol: p.symbol,
                size: p.positionAmt,
            ...
            const params = {
                ...
                side: 'SELL',
                type: 'MARKET',
                // One-Way Mode: positionSide 파라미터 제거
                quantity: qtyString,
                timestamp: timestamp.toString(),
                recvWindow: '5000'
            };
    ...
    async closeShortPosition(symbol, quantity) {
        try {
            if (!this.apiKey) {
                throw new Error('Binance API key not configured');
            }
    ...
    async getFuturesPositions() {
        try {
            ...
            return positions.filter((pos) => parseFloat(pos.positionAmt) !== 0).map((pos) => {
