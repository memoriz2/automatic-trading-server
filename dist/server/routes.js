 ...
 */
async function updatePositionExitPrice(positionId) {
    try {
        ...
        const upbitSellTrades = trades.filter((t) => t.exchange === 'upbit' && t.side === 'sell');
        const binanceCoverTrades = trades.filter((t) => t.exchange === 'binance' && t.side === 'cover');
        ...
        binanceCoverTrades.forEach((trade) => {
            const qty = parseFloat(String(trade.quantity));
            const price = parseFloat(String(trade.price));
...
async function executeRealLiquidation(userId, position) {
    try {
        ...
        const bothSuccess = !!(upbitSuccess && binanceSuccess);
        const partialSuccess = !!(upbitSuccess && !binanceSuccess) || !!(!upbitSuccess && binanceSuccess);
        // 부분 청산 시 포지션 수량 업데이트
        if (partialSuccess) {
            const updateData = {};
            if (upbitSuccess) {
...
export async function registerRoutes(app, server) {
    ...
    app.post("/api/positions/:id/close", authenticateSession, async (req, res) => {
        try {
    ...
    // 포지션 조회 API
    // 포지션 히스토리 조회 (닫힌 포지션만)
    app.get("/api/positions/history", authMiddleware, async (req, res) => {
        try {
            const userId = req.user.id;
            const limit = parseInt(req.query.limit) || 100;
    ...
    // 기본: status=open 만 반환
    app.get("/api/me/positions", authenticateSession, async (req, res) => {
        try {
            const userId = req.user.id;
    ...
    // 강제진입 설정 조회 API
    app.get("/api/force-entry-settings", authenticateSession, async (req, res) => {
        try {
            const userId = req.user.id;
            const result = await storage.pool.query(`SELECT
          investment_amount,
          leverage,
          take_profit_offset,
          created_at
    ...
    app.post("/api/force-entry-settings", authenticateSession, async (req, res) => {
        try {
            const userId = req.user.id;
            ...
            await storage.pool.query(`INSERT INTO force_entry_settings
          (user_id, investment_amount, leverage, take_profit_offset, created_at, updated_at)
         VALUES ($1, $2, $3, $4, NOW(), NOW())`, [userId, amount, lev, offset]);
        ...
        try {
            ...
            try {
              ...
              `, [
                    ...
                    activePosition = activePositions.find((p) => p.symbol === symbol && p.status === 'open');
                    if (activePosition) {
            ...
            try {
                ...
                if (!finalPositionId) {
                    ...
                    if (!finalPositionId) {
                        ...
                        const matchingPosition = userPositions.find((p) => p.symbol === symbolOnly &&
                            p.status === 'open' &&
...
(async () => {
  ...
  try {
    ...
    app.post("/api/trading/binance/short", authenticateSession, async (req, res) => {
        try {
            ...
            `, [
                ...
                // 강제진입: 사용자의 가장 최근 open 포지션 찾기
                const activePositions = await storage.getActivePositions(userId);
                activePosition = activePositions.find((p) => p.symbol === symbolOnly && p.status === 'open');
            }
            if (activePosition) {
    ...
    // AI 인사이트 최신 조회
    app.get("/api/ai-insights/latest", authenticateSession, async (req, res) => {
        try {
            const userId = req.session?.userId;
            const result = await storage.pool.query(`SELECT id, insight_type, insight_text, created_at
         FROM ai_insights
         WHERE user_id = $1
         ORDER BY created_at DESC
         LIMIT 1`, [userId]);
    ...
    app.get("/api/recommended-strategies", authenticateSession, async (_req, res) => {
        try {
            // 10개 전략 생성
            const strategies = [
